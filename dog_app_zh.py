...
print(np.mean([face_detector(human) for human in dog_files_short]))
...
# 
# __回答:__要求用户提供清晰面部特征的人脸图像是不太合理的，用户不一定能够提供和清晰的图片，这时需要我们的算法更具有鲁棒性，能够在不是太清楚的的图片中检测到人脸，所以CNN神经网络是一个很好的办法。
# 
# 
...
print(np.mean([dog_detector(dog) for dog in human_files_short]))
...
# 
# __回答:__ 
# 1）我需要加两层卷积层进一步提取狗狗图片的细节特征，有利于提高准确率；新增的过滤器数量为128和256，同时我在每一个卷积层中使用了零填充。
# 2）卷积层可以对图片中的局部位置进行感知，对各个局部位置之间的关系进行学习，从而得到全局的整体信息，池化层的作用在于特征降维，减少过拟合，最大池化层能够保留最强的特征，抛弃其他弱的特征，全局平均池化是将最后一层的特征进行平局化，形成一个特征点，将在这些特征点进行softmax提取多类别中每一个类别的概率。
...
model.add(Conv2D(filters=32,kernel_size=2,padding='same',kernel_initializer='random_uniform',bias_initializer = 'zero'))
model.add(MaxPooling2D(pool_size=2))
model.add(BatchNormalization())
model.add(Activation('relu'))
...
# 
# 
# __回答:__ 将Xception得最后一层的输出作为全局平均池化层的输入，并增加一个全连接层，使用Softmax激活函数，对狗狗属于133种数目的类别进行概率分配，得到属于某一类的最大概率。
# 1）这个架构在这一个分类任务中成功的原因：该构架已经训练好了优良的参数，并且该架构训练到了2048维度的向量，训练的深度很高，准确度相当高，并且在卷积层中所有的激活函数都是relu，避免了梯度消失的问题。2）早期的架构不成果的原因是其没有经过大量数据集并且训练很长的时间得到优良的参数，并且Xception已经在众多图像识别领域中拔得头筹，证明其已经经过无数的修正和考验，效果固然可靠。
...
def predict_label(img_path):
    ...
    if face_detector(img_path) > 0:
        ...
        print("Hello, Dog")
        print('You look like a %s in dog world'% Xception_predict_breed(img_path))
    else:
        print("Error Input")
...
# 
# 
# 1.比我预想的好一些，8各图片中至少有7个分类正确了，使用猫的图片时，也给出了我既不是人也不是狗的结果，但是还是将一个狗狗识别成了人。
# 2.三点想法如下：
# 1）更多的数据，对数据集进行数据增强，这样可以提高模型的鲁棒性
# 2）使用集成学习的办法，通过和之前学习的机器学习中的bagging的方法对模型进行投票得到投票率最高的结果；
# 3）使用更加优化的模型，可以使用一些更加先进的目标识别算法，如RCNN等等。
...
images_paths = get_all_files('test_images')
